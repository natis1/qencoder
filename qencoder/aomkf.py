...
def aom_keyframes(video_path: Path, stat_file, min_scene_len, ffmpeg_pipe, encoder, netThreads, video_params, qinterface):
    ...
    if (encoder == "aom"):
        ...
        e = f'--threads={str(netThreads)} --cpu-used=5 --end-usage=q --cq-level=40'
    ...
    while True:
        ...
        if match:
            ...
            # Do not send too many signals.
            stop = datetime.datetime.now()
            elapsed = stop - start
            if (elapsed > datetime.timedelta(milliseconds=100)):
                start = datetime.datetime.now()
                qinterface.q.put([0, "Analysis: " + str(new) + "/" + str(total), math.floor(10 * new / total)])
            frame = new
    ...
    sleep(0.05)
    qinterface.q.put([0, "Analysis Done", 10])
